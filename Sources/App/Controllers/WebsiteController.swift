...
struct WebsiteController: RouteCollection {
    func boot(router: Router) throws {
        ...
        router.get("categories", use: allCategoiesHandler)
        router.get("categories", Category.parameter, use: categoryHandler)
        router.get("acronyms", "create", use: createAcronynmHandler)
    ...
    func allCategoiesHandler(_ req: Request) throws -> Future<View> {
    ...
    func createAcronynmHandler(_ req: Request) throws -> Future<View> {
    ...
    func createAcronymPostHandler(_ req: Request, data: CreateAcronymData) throws -> Future<Response> {
        let acronnym = Acronym(short: data.short, long: data.long, userID: data.userID)
        return acronnym.save(on: req).flatMap(to: Response.self) { acronym in
            guard let id = acronnym.id else {
                throw Abort(.internalServerError)
            }
            
            var categorySaves: [Future<Void>] = []
            for category in data.categories ?? [] {
                try categorySaves.append(Category.addCategory(category, to: acronnym, on: req))
            }
